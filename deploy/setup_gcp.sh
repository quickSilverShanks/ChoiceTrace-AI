...
echo "1. Run Cloud Build pipeline to build and deploy:"
echo "   gcloud builds submit --config=deploy/cloudbuild.yaml --substitutions=_REGION=$REGION,_REPOSITORY=$REPO_NAME"
echo "2. Expose the services publicly (if blocked by default policy constraints):"
echo "   gcloud run services add-iam-policy-binding choicetrace-frontend --member=\"allUsers\" --role=\"roles/run.invoker\" --region=$REGION"
echo "   gcloud run services add-iam-policy-binding choicetrace-backend --member=\"allUsers\" --role=\"roles/run.invoker\" --region=$REGION"
echo "========================================================"
